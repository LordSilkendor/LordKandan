...
const save=()=>{const current=projects.find(p=>p.id===activeProjectId);if(current){current.columns=data;current.categories=[...categories];current.colorMap={...categoryColorMap}}localStorage.setItem('lords-projects',JSON.stringify(projects));localStorage.setItem('lords-active-project',activeProjectId);localStorage.setItem('lords-categories',JSON.stringify(categories));localStorage.setItem('lords-category-colors',JSON.stringify(categoryColorMap));if(cloudReady){clearTimeout(cloudSaveTimer);cloudSaveTimer=setTimeout(saveCurrentProjectToCloud,350)}};
  ...
  board.innerHTML=data.map(col=>{
    ...
    return `<article class="column ${col.id===activeColumn?'mobile-active':''}" data-column="${col.id}">
      ...
      <div class="card-list" data-list="${col.id}">${visible.length?visible.map(card=>`<article class="task-card" draggable="${canEdit()}" data-id="${card.id}">
        ...
        <footer class="card-meta"><span class="card-subtasks">${card.subtasks&&card.subtasks.length?`☑ <strong>${card.subtasks.filter(s=>s.done).length}/${card.subtasks.length}</strong><span>· ${progress(card)}%</span>`:'Ingen underoppgaver'}</span>${canEdit()?`<button class="card-action" data-actions="${card.id}" aria-label="Handlinger for ${card.title}">•••</button>`:''}</footer>
...
function canEdit(){const p=projects.find(x=>x.id===activeProjectId);return isGlobalAdmin||!cloudReady||!p||p.role==='admin'||p.role==='editor'}
function canAdmin(){const p=projects.find(x=>x.id===activeProjectId);return isGlobalAdmin||!cloudReady||!p||p.role==='admin'}
function applyPermissions(){const edit=canEdit(),admin=canAdmin();$('#newTaskButton').hidden=!edit;$('#manageCategories').hidden=!edit;$('#newProject').hidden=cloudReady&&!currentUser;$('#renameProject').hidden=!admin;$('#deleteProject').hidden=!admin;$('#manageAccess').hidden=!admin}
...
function selectProject(id){activeProjectId=id;const project=projects.find(p=>p.id===id);data=project.columns;categories=project.categories&&project.categories.length?[...project.categories]:categories;categoryColorMap=project.colorMap||categoryColorMap;activeColumn=data[0].id;save();renderProjects();renderCategoryManager();renderFilters();render();closeProjectMenu()}
...
function closeModal(el=modal){el.hidden=true;const anotherOpen=[modal,actionModal,categoryModal,categoryDialog,projectDialog,subtaskDialog,taskDeleteDialog,$('#accessModal')].some(item=>!item.hidden);document.body.style.overflow=anotherOpen?'hidden':'';if(el===modal)form.reset()}
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'){if(!$('#accessModal').hidden)closeModal($('#accessModal'));else if(!taskDeleteDialog.hidden)closeModal(taskDeleteDialog);else if(!categoryDialog.hidden)closeModal(categoryDialog);else if(!subtaskDialog.hidden)closeModal(subtaskDialog);else if(!projectDialog.hidden)closeModal(projectDialog);else if(!categoryModal.hidden)closeModal(categoryModal);else if(!actionModal.hidden)closeModal(actionModal);else if(!modal.hidden)closeModal();else closeProjectMenu()}});

async function saveCurrentProjectToCloud(){if(!cloudReady||!currentUser)return;const project=projects.find(p=>p.id===activeProjectId);if(!project||String(project.id).startsWith('project-'))return;const payload={columns:project.columns,categories:project.categories||categories,colorMap:project.colorMap||categoryColorMap};const {error}=await cloud.from('projects').update({name:project.name,board:payload,updated_at:new Date().toISOString()}).eq('id',project.id);if(error)toast('Kunne ikke lagre: '+error.message)}
async function loadCloudProjects(){cloudReady=false;const profileResult=await cloud.from('profiles').select('global_role').eq('id',currentUser.id).maybeSingle();isGlobalAdmin=profileResult.data&&profileResult.data.global_role==='admin';let result=await cloud.from('projects').select('id,name,board').order('created_at');if(result.error){toast(result.error.message);return}if(!result.data.length){for(const local of projects){const payload={columns:local.columns,categories:local.categories||categories,colorMap:local.colorMap||categoryColorMap};await cloud.rpc('create_project',{project_name:local.name,project_board:payload})}result=await cloud.from('projects').select('id,name,board').order('created_at')}const memberResult=await cloud.from('project_members').select('project_id,role').eq('user_id',currentUser.id),roles={};(memberResult.data||[]).forEach(m=>roles[m.project_id]=m.role);projects=(result.data||[]).map(row=>{const boardData=row.board||{},columns=Array.isArray(boardData)?boardData:boardData.columns;return {id:row.id,name:row.name,columns:columns||[],categories:boardData.categories||categories,colorMap:boardData.colorMap||categoryColorMap,role:roles[row.id]||(isGlobalAdmin?'admin':'viewer')}});if(!projects.length){toast('Ingen prosjekter er tilgjengelige');return}activeProjectId=projects.some(p=>p.id===activeProjectId)?activeProjectId:projects[0].id;cloudReady=true;selectProject(activeProjectId)}
...
$('#loginForm').addEventListener('submit',async e=>{e.preventDefault();const email=$('#loginEmail').value.trim(),status=$('#authStatus');status.textContent='Sender innloggingslenke …';const {error}=await cloud.auth.signInWithOtp({email,options:{emailRedirectTo:window.LORDS_SUPABASE.siteUrl}});status.textContent=error?error.message:'Sjekk e-posten din og åpne innloggingslenken.'});
$('#accountButton').addEventListener('click',async()=>{await cloud.auth.signOut();location.reload()});

async function loadAccessList(){const {data:members,error}=await cloud.rpc('list_project_members',{pid:activeProjectId});if(error){toast(error.message);return}const labels={viewer:'Leser',editor:'Redaktør',admin:'Administrator'};$('#accessList').innerHTML=members.map(m=>`<div class="member-row"><span class="member-email">${m.email}</span><span class="role-badge">${labels[m.role]}</span>${m.user_id===currentUser.id?'':`<button class="remove-member" data-remove-member="${m.user_id}" aria-label="Fjern tilgang">×</button>`}</div>`).join('')}
$('#manageAccess').addEventListener('click',async()=>{closeProjectMenu();$('#accessModal').hidden=false;document.body.style.overflow='hidden';await loadAccessList()});
$('#closeAccess').addEventListener('click',()=>closeModal($('#accessModal')));
$('#accessModal').addEventListener('click',e=>{if(e.target===$('#accessModal'))closeModal($('#accessModal'))});
$('#accessForm').addEventListener('submit',async e=>{e.preventDefault();const {error}=await cloud.rpc('set_project_member',{pid:activeProjectId,member_email:$('#memberEmail').value.trim(),member_role:$('#memberRole').value});if(error){toast(error.message);return}$('#memberEmail').value='';toast('Tilgangen ble oppdatert');await loadAccessList()});
$('#accessList').addEventListener('click',async e=>{const b=e.target.closest('[data-remove-member]');if(!b)return;const {error}=await cloud.rpc('remove_project_member',{pid:activeProjectId,member_user_id:b.dataset.removeMember});if(error)toast(error.message);else{toast('Tilgangen ble fjernet');await loadAccessList()}});
